...
export default function Article() {
  ...
  return (
    <PageLayout>
      <article className="article-page intro-appear">
        <Link href="/posts" className="back-link"><ArrowLeft size={16} /> 全部笔记</Link>
        <p className="eyebrow">{post.category}</p><h1>{post.title}</h1>
        <div className="article-meta"><span><CalendarDays size={14} /> {post.date}</span><i>·</i><span><Clock3 size={14} /> {post.readingTime}</span></div>
